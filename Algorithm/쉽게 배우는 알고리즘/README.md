...
<h2><a id="8">:pencil2: Chapter8. 집합의 처리</a></h2>

**:pushpin: 연결 리스트를 이용한 집합의 처리**
  
연결 리스트를 이용한 표현에서는 각 원소당 하나의 노드를 만들고 이들을 연결 리스트로 연결함.<br>
각 노드에는 원소를 저장하는 필드와 다음 원소, 대표 원소를 가리키는 두 개의 포인터가 있음.<br>
다음 원소를 가리키는 포인터를 이용해 집합의 모든 원소가 연결 리스트로 연결됨.<br>
각 집합의 대표 원소는 연결 리스트의 맨 앞에 있는 원소가 됨.<br>
각 집합에는 마지막 원소를 가리키는 tail이라는 변수를 두는데, 이 변수는 두 집합을 합칠 때 시간을 절약할 수 있게 해줌.<br>
상호 배타적 집합의 관리를 위해 필요한 연산으로는 Make-set(x), Find-set(x), Union(x, y)가 있음.<br>
<br>
Make-set(x)는 원소 하나로 구성된 집합을 만드는데, 노드를 하나 만들어 해당 원소를 저장함.<br>
대표 노드로는 자신을 가리키도록 하고, 다음 원소는 없으므로 포인터는 NIL로 해둠.<br>
find-set(x)는 원소 x가 포함된 집합을 알아냄.<br>
원소 x가 가리키는 대표 노드를 리턴함.<br>
union(x,y)는 원소 x가 속한 집합과 원소 y가 속한 집합을 하나로 합치는 연산임.<br>
우선 find-set(x)와 find-set(y)를 이용하여 x와 y가 속한 집합의 대표 노드를 각각 알아낸 다음 두 집합 중 하나를 다른 집합 뒤에 붙임.<br>
뒤에 붙는 집합을 부집합, 앞쪽에 있는 집합을 주집합이라고 함.<br>
주집합의 tail 노드의 다음 원소 포인터를 부집합의 대표 노드를 가리키도록 바꾸면 두 리스트가 하나로 합쳐짐.<br>
부집합 모든 노드의 대표 원소 포인터는 주집합의 대표 노드를 가리키도록 바꿈.<br>
<br>
make-set은 하나의 원소로 된 집합을 초기화하는 것이므로 상수 시간이 들음.<br>
find-set도 상수 시간이 들음.<br>
해당 원소의 노드에 있는 대표 원소만 리턴하면 되기 때문임.<br>
union을 할 때 시간이 가장 많이 드는 작업은 대표 원소를 가리키는 포인터를 바꿔주는 일임.<br>
큰 집합을 그대로 두고 작은 집합을 붙이는 것이 대표 원소 포인터를 갱신하는 비용이 덜 들 것임.<br>
이런 방식으로 두 집합을 합치는 것을 무게를 고려한 Union이라고 함.<br>
다음은 상각 분석임.<br>

<pre>
연결 리스트를 이용해 표현되는 배타적 집합에서 무게를 고려한 Union을 사용할 때, m번의 make-set, union, find-set 중 n번이 make-set이라면 이들의 총 수행 시간은 O(m + nlogn)임.
</pre>

n번의 make-set을 포함하므로 원소의 총수는 n개임.<br>
amke-set과 find-set은 상수 시간이 소요되고, 이들을 합해도 m번을 넘지 않으므로 O(m)임.<br>
무게를 고려하는 Union이므로 항상 작은 집합이 큰 집합 뒤에 붙음.<br>
임의의 원소 x에 대해 x가 큰 집합에 속한다면 x의 대표 노드 포인터를 갱신할 필요가 없음.<br>
그러나 x가 작은 집합에 속하면 해당 집합의 노드에 한 번씩 갱신이 발생하므로 x의 노드에도 한 번의 갱신이 발생함.<br>
이 때, 두 집합의 크기가 같으면 둘 중 한쪽에 대표 노드 갱신이 필요하므로 이 때도 x의 노드에 갱신이 발생할 수 있음.<br>
갱신이 한 번 발생할 때마다 x가 속한 집합의 크기는 최소한 두 배 이상 커짐.<br>
x가 둘 중 크지 않은 집합에 속할 때만 갱신이 일어나므로, 갱신이 일어났다는 것은 x가 원래 속한 집합보다 작지 않은 집합과 더해졌음을 의미함.<br>
하지만 원소가 총 n개이므로 집합의 크기가 n을 넘을 수는 없음.<br>
즉, 원소 x의 대표 원소 포인터 갱신은 기껏해야 ⌈log2n⌉임.<br>
원소의 총수는 n이므로 모든 원소를 고려하면 대표 노드 포인터의 갱신 총 비용은 기껏해야 ⌈nlog2n⌉을 넘지 않음.<br>
그러므로 전체 시간은 O(m+nlogn)임.<br>
  
**:pushpin: 트리를 이용한 집합의 처리**

각 집합을 하나의 트리로 표현하는 방법임.<br>
흔히 다루는 트리와는 조금 다른 방식의 표현을 사용함.<br>
트리를 나타낼 때 보통은 부모 노드가 자식 노드를 가리키도록 하지만 여기서는 자식 노드가 부모 노드를 가리키도록 함.<br>
임의의 노드에서 부모를 가리키는 포인터를 계속 따라가다 보면 자신이 속한 트리의 루트 노드를 만나게 됨.<br>
이 루트 노드가 집합의 대표 노드임.<br>
루트 노드의 부모 노드 포인터는 자기 자신을 가리킴.<br>
트리를 이용한 표현에서 두 집합을 합치는 방법은 두 집합 중 한 집합의 루트가 다른 집합의 루트를 가리키도록 포인터 하나만 변경해주면 됨.<br>
...
**랭크를 이용한 union**<br>
각 노드는 자신을 루트로 하는 서브 트리의 높이의 상한을 랭크라는 이름으로 저장함.<br>
단 하나의 노드로 된 트리의 높이는 0이라고 정의함.<br>
루트 노드의 랭크가 해당 집합의 랭크가 됨.<br>
랭크를 이용한 union에서는 두 집합을 합칠 때 랭크가 낮은 집합을 랭크가 높은 집합에 붙임.<br>
랭크가 높은 집합에 랭크가 낮은 집합을 붙이면 랭크는 랭크가 높은 집합의 랭크와 동일함.<br>
합집합의 랭크가 커지는 경우는 두 집합의 랭크가 동일한 경우밖에 없음.<br>
...
경로 압축은 find-set을 행하는 과정에 경로의 길이를 줄이는 아이디어를 넣은 것임.<br>
find-set(x)을 수행하는 과정에서 만나는 모든 노드가 직접 루트를 가리키도록 포인터를 바꿔줌.<br>
이렇게 함으로써 트리의 랭크를 줄일 가능성이 높음.<br>
...
**정리 8-2**<br>
랭크를 이용한 union을 사용하면 랭크가 k인 노드를 대표로 하는 집합의 원소 수는 최소한 2^k개임.<br>
<br>
랭크가 0이면 원소가 1개임.<br>
2^0 = 1이므로 k=0에서 정리가 만족됨.<br>
랭크가 r일 때 최소한 2^r개의 원소를 갖는다고 가정함.<br>
랭크가 r인 트리와 랭크가 r보다 작은 트리를 합치면 랭크는 여전히 r이면서 원소 수만 늘어남.<br>
오직 랭크가 r로 같은 트리가 합쳐져야만 랭크가 증가함.<br>
가정에 따라 이들 각각이 최소한 2^r개의 원소를 가지므로 이들을 합치면 최소한 2^(r+1)개의 원소를 가짐.<br>
따라서 랭크가 k인 집합의 원소 수는 최소한 2^k개임.<br>

**정리 8-3**<br>
랭크를 이용한 union을 사용하면, 원소 수가 n인 집합을 표현하는 트리에서 임의의 노드의 랭크는 O(logn)임.<br>
<br>
정리 8-2로 증명됨.<br>

**정리 8-4**<br>
트리를 이용해 표현되는 배타적 집합에서 랭크를 이용한 union을 사용할 때, m번의 make-set, union, find-set 중 n번이 make-set이라면, 이들의 총 수행 시간은 O(mlogn)임.<br>
<br>
make-set과 union의 수행 시간은 O(1)임.<br>
make-set이 n번 행해졌으므로 원소의 총 수는 n임.<br>
정리 8-3에 따르면 임의 노드의 랭크는 O(logn)이므로 find-set의 수행 시간은 O(logn)임.<br>
그러므로 m번의 make-set, union, find-set에 소요되는 시간은 O(mlogn)임.<br>

**정리 8-5**<br>
트리를 이용해 표현되는 배타적 집합에서 랭크를 이용한 union과 경로 압축을 이용한 find-set을 동시에 사용하면, m번의 make-set, union, find-set 중 n번이 make-set일 때 이들의 수행 시간은 O(mlog*n)임.<br>
<br>
log*n = min{k|logloglog...logn<=1}임.<br>
즉, n에 log를 계속 적용할 때 k번을 적용하면 최초로 1이하로 된다는 뜻임.<br>
이것은 현실적으로 다루는 거의 모든 수에서 사실상 상수로 볼 수 있을 정도로 작음.<br>
예를 들어, 2^65536은 5임.<br>
log*n은 사실상 O(1)임.<br>
이 정리가 말하는 것은 m번의 연산을 할 경우에 최악의 경우 O(m)의 시간이 든다는 것임.<br>
경로 압축 시 영향을 받는 노드의 랭크 값을 수정해주는 것이 바람직하나 거의 불가능함.<br>
그래서 랭크 값이 실제로 바뀌어도 그대로 둠.<br>
