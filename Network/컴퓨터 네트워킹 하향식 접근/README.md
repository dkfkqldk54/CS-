...
신뢰적인 데이터 전달
TCP 연결시 소켓별로 버퍼가 생성이 됨
프로세스 A와 B는 각각 송신 버퍼와 수신 버퍼를 가지고 있음
버퍼는 애플리케이션에서 내려오는 데이터의 속도와 내보내는 속도가 맞지 않아서 완충하기 위해 있음
프로세스가 자신의 수신 버퍼로부터 읽은 데이터 스트림이 손상되지 않았으며, 손실이나 중복이 없고, 순서가 유지됨을 보장함
TCP는 재전송마다 마지막 estimatedRTT와 devRTT로부터 타임아웃을 가져오는 것이 아니라 타임아웃 주기를 이전 값의 두 배로 설정함
새로 타이머를 갱신할 경우에는 다시 estimatedRTT와 devRTT의 최근 값을 사용함

빠른 재전송
중복 ACK를 이용함
3개의 중복 ACK를 받으면 3개의 세그먼트가 분실되었음을 의미함
이 경우 타이머가 만료되기 전에 손실 세그먼트를 재전송함

TCP 흐름제어

수신자의 버퍼가 오버플로 되지 않도록 수신하는 애플리케이션이 읽는 속도와 송신자가 송신하는 속도를 같게 함
rwnd = 수신 버퍼 - (마지막으로 읽힌 데이터 스트림의 마지막 바이트 수 - 마지막으로 저장된 데이터 스트림의 마지막 바이트 수)
수신 측 호스트는 송신 측 호스트에게 전송하는 모든 세그먼트의 윈도우 필드에 현재 rwnd 값을 설정함
호스트는 LastByteSent와 LastByteAcked 값을 유지함. 이 두 변수의 차이는 연결에 전송 확인 응답이 안 된 데이터의 양임
이를 rwnd보다 작게 유지하여 오버플로를 방지함
rwnd가 0이고, 수신 호스트가 송신 호스트에게 전송할 것이 없는 경우, 버퍼가 비워져도 알릴 방법이 없음
수신 윈도우가 0일 때, 바이트 데이터를 보내 버퍼가 비워질 때 긍정 확인 응답을 받아냄

TCP 연결 관리

1. SYN 세그먼트를 클라이언트에서 서버로 보낸다. 이 때 최초의 순서번호 client-isn을 선택한다.(SYN)
2. 클라이언트는 TCP SYN 세그먼트를 받고, TCP 버퍼와 변수를 할당한다. 확인응답 필드는 client-isn + 1이고, server-isn을 자신의 순서번호 필드에 넣는다.(SYN+ACK)
3. SYN+ACK 수신 후 클라이언트는 버퍼와 변수를 할당한다. 확인응답 필드에 server-isn + 1을 넣어 보내고, 연결이 되었기 때문에 SYN 비트는 0으로 해서 보낸다. 페이로드 데이터가 동반될 수 있다.(ACK)
클라이언트는 연결 종료시 FIN 플래그 비트를 1로 설정해서 보낸다.
이것을 서버가 수신하면 상대방에게 확인 세그먼트를 보내고, FIN 비트를 1로한 자신의 세그먼트를 송신한다.
클라이언트는 서버에 확인응답을 한다.
클라이언트는 마지막 ACK 응답을 보내고, 대기시간 후에 자원을 해제하는데, 이는 ACK가 손실된 경우에 마지막 확인응답을 재전송하기 위함이다.
이 모든 과정은 반대로도 가능하다.

TCP 혼잡제어

혼잡제어의 원인과 비용
혼잡은 네트워크가 처리할 수 있는 양보다 많은 데이터가 들어왔을 때 생기는 현상임
패킷의 도착률이 링크 용량에 근접하면 큐잉 지연이 커짐
송신자는 버퍼 오버플로 때문에 버려진 패킷을 보상하기 위해 재전송해야 함
불필요한 재전송은 라우터가 패킷의 불필요한 복사본을 전송하는데 링크 대역폭을 사용하는 원인임
패킷이 경로 상에서 버려질 때, 버려지는 지점까지 패킷을 전송하는데 사용된 상위 라우터에서 사용된 전송량은 헛된 것임

IP 계층은 네트워크 혼잡에 관해서 종단 시스템에게 어떤 직접적인 피드백도 하지 않음
TCP 송신자는 혼잡 윈도우 cwnd로 트래픽 전송률을 제한함. RTT 이후에 확인응답을 수신하므로 대략적인 송신률은 cwnd/RTT 바이트/초임
TCP는 타임아웃 혹은 3개의 중복된 ACK를 수신하는 손실 이벤트를 통해 혼잡을 감지함
TCP 송신자는 손실 이벤트가 발생할 때까지는 ACK가 도착함에 따라 전송률을 증가시키고, 손실 이벤트가 발생하면 전송률을 줄임

TCP 혼잡제어 알고리즘

슬로 스타트
TCP 연결 시작시 cwnd 값을 일반적으로  1MSS로 초기화함
따라서 초기 전송률은 대략 MSS/RTT임
확인응답을 받을 때마다 각 세그먼트에 대해 1MSS씩 윈도우를 증가시킴. 즉 통상 2배씩 증가함
첫 번째 타임아웃이 있을 경우 cwnd를 1로 하고, 새로운 슬로 스타트를 시작하고, slow start threshold를 1/2로 정함
ssthresh에 도달하면 슬로 스타트를 종료하고 혼잡 회피 모드로 전환함
3개의 중복 ACK 발견시 빠른 회복 상태로 들어감

혼잡 회피
혼잡 회피 시점에서 cwnd는 마지막 시점의 대략 절반임
매 RTT마다 2배가 아닌 1MSS만큼 cwnd를 증가시킴
선형증가는 타임아웃이 발생한 경우 cwnd를 1로하고, ssthresh를 당시의 절반으로 설정함
3개의 중복 ACK에 의한 경우는 ssthresh를 3개의 중복 ACK를 수신한 시점의 cwnd의 반으로 하고, 빠른 회복으로 들어감
이 때, cwnd는 ssthresh에서 3MSS를 더한 값임

빠른 회복
TCP를 빠른 회복 상태로 들어가게 했던 세그먼트에 대한 중복된 ACK를 수신할 때마다 1MSS만큼 cwnd 증가함
