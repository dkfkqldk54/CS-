...
B트리에서 항상 거쳐야 하는 노드는 루트 노드가 있는 노드일 것이다. 루트 노드가 있는 블록은 메인 메모리에 실어 놓으면, B트리를 꺼내 쓸 때마다 디스크 접근 횟수를 1회씩 줄일 수 있을 것이다.<br>
디스크 접근 횟수를 더 줄이고 싶다면, 용량이 허락하는 깊이까지 B트리의 노드들을 메인 메모리에 실어놓고, 그 외의 블록들은 디스크에 저장해서 사용하는 방법도 있을 수 있다. 이렇게 하면 메인 메모리에 실은 깊이만큼 디스크 접근 횟수를 줄일 수 있을 것이다.<br>
